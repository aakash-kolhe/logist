    ...
    if(mysqli_num_rows($select_consignor_run) > 0)
    {
    	...
    	while($select_consignor_res = mysqli_fetch_array($select_consignor_run))
    	{
    		...
    		<?php if(!empty($select_consignor_res['contact_mstr_first_Name'])) { ?>
    			...
    			<option value="<?= $select_consignor_res['contact_Id']?>"
	                <?php if(trim($select_consignor_res['contact_Id']) == trim($ConsignmentDetailsRes['consignor']) ){ echo 'selected'; } ?>>
	                <?php echo $select_consignor_res['contact_mstr_first_Name'].' '.$select_consignor_res['contact_mstr_last_Name']?>
	            </option>
