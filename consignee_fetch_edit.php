    ...
    if(mysqli_num_rows($select_consignee_run) > 0)
    {
    	...
    	while($select_consignee_res = mysqli_fetch_array($select_consignee_run))
    	{
    		...
    		<?php if(!empty($select_consignee_res['contact_mstr_first_Name'])) { ?>
    			...
    			<option value="<?= $select_consignee_res['contact_Id']?>"
                    <?php if(trim($select_consignee_res['contact_Id']) == trim($ConsignmentDetailsRes['consignee']) ){ echo 'selected'; } ?>>
                    <?php echo $select_consignee_res['contact_mstr_first_Name'].' '.$select_consignee_res['contact_mstr_last_Name']?>
                </option>
